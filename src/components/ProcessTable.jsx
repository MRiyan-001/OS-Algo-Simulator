...
const ProcessTable = ({ onRemoveProcess, processes }) => {
  return processes.length === 0 ? (
    ...
    <div className="p-6 shadow-[0_2px_8px_0px_rgba(99,99,99,0.2)] border-border/50 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="border-b border-b-gray-300 bg-gray-200 text-gray-600">
            <tr className="border-border/50 text-start hover:bg-[hsl(220_15%_60%)]/10">
              <th className="font-semibold px-4 py-3 text-start ">Process</th>
              <th className="font-semibold px-4 py-3 text-start ">Arrival Time</th>
              <th className="font-semibold px-4 py-3 text-start ">Burst Time</th>
              <th className="font-semibold px-4 py-3 text-start ">Priority</th>
              <th className="font-semibold text-right px-4 py-3 ">Action</th>
            </tr>
          </thead>
          <tbody>
            {processes?.map((process) => (
              <tr
                key={process.id}
                className="hover:bg-[hsl(220_15%_60%)]/10  transition-colors text-gray-500 border border-gray-200"
              >
